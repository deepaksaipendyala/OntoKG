...
class MaterialsDiscoveryEngine:
    ...
    def _extract_materials_data(self, kg_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if isinstance(kg_data, list):
            ...
            for item in kg_data:
                if isinstance(item, dict) and 'nodes' in item:
                    nodes = item['nodes']
                    if isinstance(nodes, dict) and 'Material' in nodes:
                        materials.extend(nodes['Material'])
                    elif isinstance(nodes, list):
                        materials.extend([n for n in nodes if n.get('type') == 'Material'])
        else:
            # Get materials from nodes
            nodes = kg_data.get('nodes', {})
            if isinstance(nodes, dict):
                materials = nodes.get('Material', [])
            else:
                materials = [n for n in nodes if isinstance(n, dict) and n.get('type') == 'Material']
        ...
        for material in materials:
            ...
            material_name = material_dict.get('name', material_dict.get('id'))
            if not material_name:
                continue
