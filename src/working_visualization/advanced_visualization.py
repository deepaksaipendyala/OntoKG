...
class AdvancedNetworkVisualizer:
    ...
    def create_3d_network_visualization(self, kg_data: Dict[str, Any], 
        ...
        # Fallback annotation if graph is empty
        if G.number_of_nodes() == 0 or G.number_of_edges() == 0:
            fig = go.Figure()
            fig.add_annotation(text="No graph data available to visualize",
                               xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
            fig.update_layout(margin=dict(l=0, r=0, t=60, b=0), paper_bgcolor='white', plot_bgcolor='white', height=700)
            return fig
        ...
        # Combine traces
        if not node_traces and not edge_traces:
            fig = go.Figure()
            fig.add_annotation(text="Graph contains no drawable nodes/edges",
                               xref="paper", yref="paper", x=0.5, y=0.5, showarrow=False)
            fig.update_layout(margin=dict(l=0, r=0, t=60, b=0), paper_bgcolor='white', plot_bgcolor='white', height=700)
            return fig
    ...
    def _build_networkx_graph(self, kg_data: Dict[str, Any]) -> nx.Graph:
        ...
        if isinstance(kg_data, list):
            # Merge list chunks into dict format
            nodes_dict = {}
        ...
        # Normalize nodes to dict[label] -> list[dict]
        nodes_dict = {}
        ...
        for node_type, nodes in nodes_dict.items():
            for node in nodes or []:
                ...
                if not node_id:
                    continue
                if node_id in G:
                    G.nodes[node_id]['properties'].update(node)
                else:
                    G.add_node(
                        node_id,
                        type=node_type,
        ...
        for edge in edges_list:
            ...
            if source and target:
                if source not in G:
                    G.add_node(source, type='Unknown', properties={'name': source}, size=20, color='#CCCCCC')
                if target not in G:
                    G.add_node(target, type='Unknown', properties={'name': target}, size=20, color='#CCCCCC')
                G.add_edge(
                    source,
                    target,
                    properties=edge,
