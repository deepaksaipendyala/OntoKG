"""
Example: Thermoelectric Materials Expansion
Demonstrates the LLM-guided expansion process for thermoelectric materials
"""
...
def run_thermoelectric_expansion():
    """Run the thermoelectric materials expansion example"""
    
    print("🔥 Thermoelectric Materials Expansion Example")
    print("=" * 50)
    ...
    try:
        ...
        # Get current thermoelectric materials
        print("\n📊 Current thermoelectric materials:")
        current_materials = kg.get_materials_for_application("thermoelectric_device")
        ...
        response = llm.generate_thermoelectric_hypotheses(material_names, context)
        ...
        for i, hypothesis_data in enumerate(response.hypotheses, 1):
            ...
            print(f"\n   {i}. {material} → thermoelectric_device")
            print(f"      Rationale: {rationale}")
            print(f"      LLM Confidence: {confidence:.2f}")
            
            # Validate hypothesis
            validation_result = validator.validate_thermoelectric_material(material)
        ...
        print(f"\n📋 Updated thermoelectric materials:")
        updated_materials = kg.get_materials_for_application("thermoelectric_device")
...
def main():
    """Main function"""
    success = run_thermoelectric_expansion()
    
    if success:
        print(f"\n🎉 Thermoelectric expansion completed successfully!")
        print(f"   You can now query the expanded knowledge graph.")
    else:
        print(f"\n❌ Thermoelectric expansion failed.")
